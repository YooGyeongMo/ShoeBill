...
namespace DBP_TeamProject.Forms
{
    ...
    public partial class MasterMaker : Form
    {
        ...
        private void button_addbig_Click(object sender, EventArgs e)
        {
            // 텍스트박스1에서 입력된 데이터 가져오기
            string inputData = textBox_big.Text;

            // 대분류를 분류_대분류 테이블에 추가하는 코드 작성
            if (!string.IsNullOrEmpty(inputData))
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    // INSERT 쿼리 작성
                    string query = $"INSERT INTO s5585452.분류_대분류 (대분류명) VALUES ('{inputData}')";
                ...
                MessageBox.Show("대분류가 추가되었습니다.", "성공", MessageBoxButtons.OK, MessageBoxIcon.Information);

                // 데이터 추가 후 대분류 콤보박스 다시 로드
        ...
        private void button_addmid_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(inputData) && selectedBigCategoryId != -1)
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    // INSERT 쿼리 작성
                    string query = $"INSERT INTO s5585452.분류_중분류 (중분류명, 대분류ID) VALUES ('{inputData}', {selectedBigCategoryId})";
        ...
        private void button_addsmall_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(inputData) && selectedmidCategoryId != -1)
            {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    string query = $"INSERT INTO s5585452.분류_소분류 (소분류명, 중분류ID) VALUES ('{inputData}', {selectedmidCategoryId})";
